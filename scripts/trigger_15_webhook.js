...
// Pega os 15 primeiros
const amostra = sheet.slice(0, 15).map(row => ({
...
const req = http.request(WEBHOOK_URL, options, (res) => {
    ...
    res.on('end', () => {
        ...
        if (res.headers['content-type'] && res.headers['content-type'].includes('spreadsheet')) {
            require('fs').writeFileSync('/home/jessegoncalves/problm/planilha_15_imoveis.xlsx', buffer);
            console.log('Planilha salva com sucesso em planilha_15_imoveis.xlsx');
        } else {
